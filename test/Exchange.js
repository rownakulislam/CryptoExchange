...
describe("Exchange", () => {
  ...
  describe("Making orders",()=>{
    ...
    describe("Success",async()=>{
      ...
      it("tracks the new created order",async()=>{
    ...
    describe("Failure",async()=>{
        it("rejects orders that have not enough balance",async()=>{
  ...
  describe ("Order actions",async()=>{
    ...
    beforeEach(async()=>{
        //deposit token before withdrawing
          ...
          transaction=await exchange.connect(user1).depositeTokens(token1.address,amount)
          result=await transaction.wait()
          //give tokens to uder2
          transaction=await token2.connect(deployer).transfer(user2.address,tokens(100))
          result=await transaction.wait()

          //aprove tokens for user2
          transaction=await token2.connect(user2).approve(exchange.address,tokens(2))
          result=await transaction.wait()
          //deposite tokens for user2
          transaction=await exchange.connect(user2).depositeTokens(token2.address,tokens(2))
          result=await transaction.wait()
    ...
    describe("cancelling orders",async()=>{
      describe("success",async()=>{
          ...
          it("updates cancelled orders",async()=>{
            expect(await exchange.orderCencelled(1)).to.equal(true)
